...
class SixDRepNet2(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
class SixDRepNet3(nn.Module):
    def __init__(self,
                 backbone_name, backbone_file, deploy,
                 pretrained=True):
        super(SixDRepNet3, self).__init__()
        repvgg_fn = get_RepVGG_func_by_name(backbone_name)
        backbone = repvgg_fn(deploy)
        if pretrained:
            checkpoint = torch.load(backbone_file)
        ...
        self.linear_reg = nn.Linear(fea_dim, 6)
        self.linear_cls = nn.Linear(fea_dim,1)
    ...
    def forward(self, x):
        ...
        x = self.layer4(x)
        x = self.gap(x)
        rx = torch.flatten(x, 1)
        x = self.linear_reg(rx)
        y=self.linear_cls(rx).reshape([-1])
        return utils.compute_rotation_matrix_from_ortho6d(x),y
    def Inference(self,x):
        ord,y=self.forward(x)
        return ord,F.sigmoid(y)