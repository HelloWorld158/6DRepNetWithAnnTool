...
class ChenZhuo(Dataset):
    def __init__(self,data_dir,transforms):
        self.data_dir=data_dir
        self.transform=transforms
        self.files=basFunc.getdatas(data_dir)
    def __len__(self):
    ...
    def __getitem__(self, index):
        ...
        box,label=dio.LoadVariablefromPKL(os.path.join(d,name+".pkl"))
        if(len(box)==0):
            ...
            y_max = box[3]
            #k=0.5
            k = np.random.random_sample() * 0.4+0.3
            cutimg=img[int(y_min):int(y_max),int(x_min):int(x_max)]
            img=utils.GetExpandImage(img,x_min,y_min,x_max,y_max,k)
        if(len(label)!=0):
        # And convert to degrees.
            pitch = label[0] # * 180 / np.pi
            yaw = label[1] #* 180 / np.pi
            roll = label[2] # * 180 / np.pi
        ...
        if self.transform is not None:
            ...
            for trans in self.transform.transforms:
                ...
                #print(cnt,trans,img.dtype,flush=True)
                #cnt+=1
                img,R = trans(img,R)
        return img,  torch.FloatTensor(R),[], self.files[index] 
...
def getDataset(dataset, data_dir, filename_list, transformations, train_mode = True):
    if dataset == 'Pose_300W_LP':
        ...
        pose_dataset = AFW(
            data_dir, filename_list, transformations)
    elif dataset == 'chenzhuo':
        pose_dataset=ChenZhuo(data_dir,transformations)
    else:
